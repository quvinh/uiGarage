...
const AppNotificationsDropdown = () => {
    ...
    const handleNotification = (code) => {
        console.log(code)
        Promise.all([getData('http://127.0.0.1:8000/api/admin/notification/showNotification/' + code)])
            .then(function (res) {
    ...
    const handleListItem = (id) => {
        Promise.all([getData('http://127.0.0.1:8000/api/admin/notification/showListItemById/' + id)])
    ...
    useEffect(() => {
        Promise.all([getData('http://127.0.0.1:8000/api/admin/notification')
        ]).then(function (res) {
            console.log(res[0].data)
        ...
        <>
            <CDropdown variant="nav-item" placement={'bottom-start'} >
                <CDropdownToggle size={'lg'} placement="bottom-end" className="py-0" caret={false}>
                    ...
                    {/* <CBadge size='sm' className="border border-light p-2" color="danger" position="top-end" shape="rounded-circle">
